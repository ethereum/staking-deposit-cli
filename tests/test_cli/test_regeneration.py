...
def test_regeneration(monkeypatch) -> None:
    ...
    # Check files
    validator_keys_folder_path_1 = os.path.join(
        folder_path_1, DEFAULT_VALIDATOR_KEYS_FOLDER_NAME
    )
    _, _, files = next(os.walk(validator_keys_folder_path_1))
    part_1_key_files = sorted(
        [key_file for key_file in files if key_file.startswith("keystore")]
    )

    all_uuid = [
        get_uuid(validator_keys_folder_path_1 + "/" + key_file)
    ...
    # Check files
    validator_keys_folder_path_2 = os.path.join(
        folder_path_2, DEFAULT_VALIDATOR_KEYS_FOLDER_NAME
    )
    _, _, files = next(os.walk(validator_keys_folder_path_2))
    part_2_key_files = sorted(
        [key_file for key_file in files if key_file.startswith("keystore")]
    )

    all_uuid = [
        get_uuid(validator_keys_folder_path_2 + "/" + key_file)
