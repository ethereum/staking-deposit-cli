...
@pytest.mark.parametrize("test", test_vectors)
def test_hkdf_mod_r(test) -> None:
    seed = bytes.fromhex(test["seed"])
    assert bls.KeyGen(seed) == _HKDF_mod_r(IKM=seed)


@pytest.mark.parametrize("seed", [b"\x00" * 32])
@pytest.mark.parametrize(
    "key_info", [b"\x00" * 32, b"\x01\x23\x45\x67\x89\xab\xbc\xde\xff", b"\xff" * 16]
)
...
@pytest.mark.parametrize("test", test_vectors)
@pytest.mark.parametrize("is_valid_seed", (True, False))
...
@pytest.mark.parametrize("test", test_vectors)
@pytest.mark.parametrize("is_valid_index", (True, False))
