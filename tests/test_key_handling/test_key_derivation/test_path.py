...
test_vector_filefolder = os.path.join(
    os.getcwd(),
    "tests",
    "test_key_handling",
    "test_key_derivation",
    "test_vectors",
    "tree_kdf_intermediate.json",
)
with open(test_vector_filefolder, "r") as f:
    test_vector_dict = json.load(f)


@pytest.mark.parametrize("test_vector", [test_vector_dict])
def test_flip_bits_256(test_vector) -> None:
    # 64 comes from string chars containing .5 bytes
    test_vector_int = int(test_vector["seed"][:64], 16)
...
def test_HKDF_mod_r(test_vector) -> None:
    ...
    test_1 = (
        bytes.fromhex(test_vector["compressed_lamport_PK"]),
...
def test_mnemonic_and_path_to_key(test_vector) -> None:
    ...
    assert (
        mnemonic_and_path_to_key(mnemonic=mnemonic, path=path, password=password) == key
    )
