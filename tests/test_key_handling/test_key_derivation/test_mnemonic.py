...
WORD_LISTS_PATH = os.path.join(
    os.getcwd(), "staking_deposit", "key_handling", "key_derivation", "word_lists"
)
all_languages = MNEMONIC_LANG_OPTIONS.keys()

test_vector_filefolder = os.path.join(
    "tests", "test_key_handling", "test_key_derivation", "test_vectors", "mnemonic.json"
)
...
def test_bip39(language: str, test: Sequence[str]) -> None:
    ...
    assert (
        get_mnemonic(
            language=language, words_path=WORD_LISTS_PATH, entropy=test_entropy
        )
        == test_mnemonic
    )
