...
test_vector_folder = os.path.join(
    os.getcwd(), "tests", "test_key_handling", "keystore_test_vectors"
)
_, _, test_vector_files = next(os.walk(test_vector_folder))  # type: ignore

test_vector_keystores = [
    Keystore.from_file(os.path.join(test_vector_folder, f)) for f in test_vector_files
]
...
def test_encrypt_decrypt_test_vectors() -> None:
    for tv in test_vector_keystores:
        ...
        keystore = (
            Pbkdf2Keystore if "pbkdf" in tv.crypto.kdf.function else ScryptKeystore
        )
...
def test_encrypt_decrypt_pbkdf2_random_iv() -> None:
    generated_keystore = Pbkdf2Keystore.encrypt(
        secret=test_vector_secret, password=test_vector_password
    )
    assert generated_keystore.decrypt(test_vector_password) == test_vector_secret


def test_encrypt_decrypt_scrypt_random_iv() -> None:
    generated_keystore = ScryptKeystore.encrypt(
        secret=test_vector_secret, password=test_vector_password
    )
    assert generated_keystore.decrypt(test_vector_password) == test_vector_secret
