...
from staking_deposit.utils.crypto import PBKDF2, SHA256
...
def _get_word_list(language: str, path: str) -> Sequence[str]:
    ...
    dirty_list = open(
        os.path.join(path, "%s.txt" % language), encoding="utf-8"
    ).readlines()
...
def determine_mnemonic_language(mnemonic: str, words_path: str) -> Sequence[str]:
    ...
    word_language_map = {
        word: lang for lang in languages for word in _get_word_list(lang, words_path)
    }
    try:
        mnemonic_list = [
            normalize("NFKC", word)[:4] for word in mnemonic.lower().split(" ")
        ]
        word_languages = [
            [
                lang
                for word, lang in word_language_map.items()
    ...
    for language in languages:
        try:
            ...
            if len(abbrev_mnemonic_list) not in range(12, 25, 3):
                return None
            word_indices = [
                _word_to_index(abbrev_word_list, word) for word in abbrev_mnemonic_list
            ]
            ...
            if _get_checksum(entropy_bits) == checksum:
                ...
                reconstructed_mnemonic = " ".join(
                    [_index_to_word(full_word_list, index) for index in word_indices]
                )
            else:
                pass
        except ValueError:
...
def get_mnemonic(
    *, language: str, words_path: str, entropy: Optional[bytes] = None
) -> str:
    """
