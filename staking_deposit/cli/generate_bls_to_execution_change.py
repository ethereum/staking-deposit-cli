...
from staking_deposit.utils.intl import closest_match, load_text
...
from .existing_mnemonic import load_mnemonic_arguments_decorator
...
@jit_option(
    ...
    help=lambda: load_text(
        ["arg_bls_to_execution_changes_folder", "help"], func=FUNC_NAME
    ),
...
@jit_option(
    callback=captive_prompt_callback(
        lambda bls_withdrawal_credentials_list: validate_bls_withdrawal_credentials_list(
            bls_withdrawal_credentials_list
        ),
        lambda: load_text(
            ["arg_bls_withdrawal_credentials_list", "prompt"], func=FUNC_NAME
        ),
    ),
    help=lambda: load_text(
        ["arg_bls_withdrawal_credentials_list", "help"], func=FUNC_NAME
    ),
...
def generate_bls_to_execution_change(
    ...
    **kwargs: Any,
    ...
    for i, credential in enumerate(credentials.credentials):
        try:
            validate_bls_withdrawal_credentials_matching(
                bls_withdrawal_credentials_list[i], credential
            )
        except ValidationError as e:
            click.echo("\n[Error] " + str(e))
            return

    btec_file = credentials.export_bls_to_execution_change_json(
        bls_to_execution_changes_folder, validator_indices
    )
    ...
    click.echo(
        load_text(["msg_creation_success"]) + str(bls_to_execution_changes_folder)
    )
