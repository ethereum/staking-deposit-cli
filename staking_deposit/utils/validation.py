...
def verify_deposit_data_json(
    filefolder: str, credentials: Sequence[Credential]
) -> bool:
    """
...
def validate_eth1_withdrawal_address(
    cts: click.Context, param: Any, address: str
) -> HexAddress:
    if address is None:
        return None
...
def verify_bls_to_execution_change_json(
    filefolder: str,
        ...
        with click.progressbar(
            ...
            return all(
                [
                    ...
                    for btec, credential, input_validator_index in zip(
                        btecs, credentials, input_validator_indices
                    )
                ]
            )
    return False
...
def validate_bls_to_execution_change(
    btec_dict: Dict[str, Any],
    ...
    *,
    ...
    genesis_validators_root = decode_hex(
        btec_dict["metadata"]["genesis_validators_root"]
    )
...
def normalize_bls_withdrawal_credentials_to_bytes(
    bls_withdrawal_credentials: str,
) -> bytes:
...
def validate_bls_withdrawal_credentials(bls_withdrawal_credentials: str) -> bytes:
    bls_withdrawal_credentials_bytes = normalize_bls_withdrawal_credentials_to_bytes(
...
def validate_bls_withdrawal_credentials_list(
    input_bls_withdrawal_credentials_list: str,
) -> Sequence[bytes]:
    bls_withdrawal_credentials_list = normalize_input_list(
        input_bls_withdrawal_credentials_list
    )
    return [
        validate_bls_withdrawal_credentials(cred)
        for cred in bls_withdrawal_credentials_list
    ]
...
def validate_bls_withdrawal_credentials_matching(
    bls_withdrawal_credentials: bytes, credential: Credential
) -> None:
